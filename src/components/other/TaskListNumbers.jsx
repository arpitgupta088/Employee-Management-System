import React from 'react'

const TaskListNumbers = ({data}) => {
    const stats = data?.taskStats || {
    newTask: 0,
    completed: 0,
    active: 0,
    failed: 0,
  };
  return (
    <div className='flex mt-10 justify-between gap-5 screen'>
        <div className='rounded-xl w-[45%] py-6 px-9 bg-yellow-400'>
            <h2 className='text-3xl text-white font-semibold'>{stats.newTask}</h2>
            <h3 className='text-xl text-white font-medium'>New Task</h3>
        </div>
        <div className='rounded-xl w-[45%] py-6 px-9 bg-blue-400'>
            <h2 className='text-3xl text-white font-semibold'>{stats.completed}</h2>
            <h3 className='text-xl text-white font-medium'>Completed Task</h3>
        </div>
        <div className='rounded-xl w-[45%] py-6 px-9 bg-green-400'>
            <h2 className='text-3xl text-white font-semibold'>{stats.active}</h2>
            <h3 className='text-xl text-white font-medium'>Accepted Task</h3>
        </div>
        <div className='rounded-xl w-[45%] py-6 px-9 bg-red-400'>
            <h2 className='text-3xl text-white font-semibold'>{stats.failed}</h2>
            <h3 className='text-xl text-white font-medium'>Failed Task</h3>
        </div>
    </div>
  )
}

export default TaskListNumbers
